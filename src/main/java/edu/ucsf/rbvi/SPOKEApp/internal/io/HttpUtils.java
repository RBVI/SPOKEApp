package edu.ucsf.rbvi.spokeApp.internal.io;
...
public class HttpUtils {
	...
	public static JSONObject getJSON(String url, Map<String, String> queryMap,
			SpokeManager manager) throws ConnectionException, SocketTimeoutException {
		return getJSON(url, queryMap, manager, 0);
	}

	@SuppressWarnings("unchecked")
	public static JSONObject getJSON(String url, Map<String, String> queryMap,
			SpokeManager manager, int timeout) throws ConnectionException, SocketTimeoutException {
...
//			return new JSONObject();
		...
		try {
			...
			jsonObject.put(SpokeManager.RESULT, obj);
	...
	public static Object testJSON(String url, Map<String, String> queryMap, SpokeManager manager,
			String json) {
	...
	public static JSONObject postJSON(String url, Map<String, String> queryMap,
			SpokeManager manager) throws ConnectionException {
		...
		try {
			...
			try {
				...
				//} catch (Exception ioe) {
					// ignore
				//}
				manager.error("Exception reading JSON from SPOKE server: \n"+ parseFailure.getMessage()+"\n Text: "+errorMessage[1]);
				throw new ConnectionException("Exception reading JSON from SPOKE server: \n"+ parseFailure.getMessage()+"\n Text: "+errorMessage[1]);
			}
	...
	public static String getStringArguments(Map<String, String> args) {
		...
		for (String key : args.keySet()) {
			// See if we've got a list
			String[] listArgs = args.get(key).split("\\|");
			if (listArgs.length > 1) {
				for (String arg: listArgs) {
					s = encodeArg(s, key, arg);
				}
			} else {
				s = encodeArg(s, key, args.get(key));
			}
		}
		return s;
	}

	private static String encodeArg(String argString, String key, String value) {
		if (!argString.equals(""))
			argString += "&";
		try {
			argString += key + "=" + URLEncoder.encode(value, StandardCharsets.UTF_8.displayName());
			return argString;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public static void addVersion(URLConnection connection, JSONObject object) {
		String api = connection.getHeaderField(SpokeManager.APIVERSION);
		if (api != null && api.length() > 0) {
			object.put(SpokeManager.APIVERSION, Integer.parseInt(api));
		}
	}
	...
	private static URLConnection executeWithRedirect(SpokeManager manager, String url, Map<String, String> queryMap) throws Exception {
	...
	private static String[] readErrorStream(InputStream stream, SpokeManager manager) throws Exception {
